...
def dom_polarity(sent_list,pos_emotion,neg_emotion,tokenizer):
    ...
    y_train=np.array([pos_tag]*1000+[neg_tag]*1000)
    print("Dominant polarity accuracy: ",np.mean(y_train==labels));     


class review_polarity:
    def __init__(self,split=0.5,reload=False,shuffle=True,remove=True,tokenize=False,wordvec=False,wordvec_file='./review_polarity_word2vec',
        ...
        self.pos_filepath="C:\\Users\\bodap\\OneDrive\\Documents\\Projects_Sem2\\NLP\\DataSets\\txt_sentoken\\pos\\"
        self.neg_filepath="C:\\Users\\bodap\\OneDrive\\Documents\\Projects_Sem2\\NLP\\DataSets\\txt_sentoken\\neg\\"

        self.pos_list=os.listdir(self.pos_filepath); #List of files
        self.neg_list=os.listdir(self.neg_filepath);#List of files
        print("Number of pos sent:",len(self.pos_list))
        print("Number of neg sent:",len(self.neg_list))

        self.sent_list=[]; 
        for _path,_list in [(self.pos_filepath,self.pos_list),(self.neg_filepath,self.neg_list)]:
            for l,file in enumerate(_list):
                with open(_path+file,'rb') as f:
                     self.sent_list.append(f.read());
        ...
        if self.word_vec:
            if wordvec_file==None:
                ...
                model.save_word2vec_format("./review_polarity_word2vec")
                return
            else:
            ...
            #self.x_train=np.zeros((len(self.sent_list),word_to_vec_dict['dog'].shape[0]));
                    ...
                    for token in tokens_in_sentence:
                        ...
                        if(token in word_to_vec_dict.keys()):
                           ...
                           sentence_dot_vector=np.dot(sentence_dot_vector,word_to_vec_dict[token]);
                            ...
                            pass
                           # print("Word missing:",token);
                    sentence_avg_vector/=(num_valid_tokens+0.1); 
            
                self.x_train[num_sample,:flength]+=sentence_avg_vector; #Sentence average 
                self.x_train[num_sample,flength:]+=sentence_dot_vector; #sentence dot.
                num_sample+=1;  
            
            self.y_train=np.array([pos_tag]*len(self.pos_list)+[neg_tag]*len(self.neg_list)) 
    ...
    def build_vocab(self):
       #builds vocabulary from teh full sent_list
        for sent in self.sent_list:
            tokens=self.tokenizer.tokenize(sent);   
            c=Counter(tokens);
...
print("Creating Review polarity class");
#reload is like recompute teh feature vectors from scratch
#instead of loading from pickle.
#"""
rp=review_polarity(dominant_polarity=True,unigrams=True,wordvec=False,wordvec_file='./review_polarity_word2vec',only_bigrams=False,reload=False,remove=False); 
data=rp.get_datasets()
#clfs=classifiers(data)
#word_vec=word_vectors(data)
clfs=classifiers(data,sqrt_norm=False,scale=True)
#"""
""" 
rp=review_polarity(unigrams=False,wordvec=True,wordvec_file='./glove.6B/glove.6B.50d.txt',
                   only_bigrams=False,reload=False,remove=False); 
data=rp.get_datasets()
#clfs=classifiers(data)
#word_vec=word_vectors(data)
...
rp=review_polarity(unigrams=True,wordvec=False,wordvec_file='./glove.6B/glove.6B.50d.txt',
                   only_bigrams=False,reload=False,remove=False,pres_or_abs=True); 
data=rp.get_datasets()
#clfs=classifiers(data)
word_vec=word_vectors(data)
clfs=classifiers(data)
...
data=rp.get_datasets()
#clfs=classifiers(data)
word_vec=word_vectors(data,method='lsa')
clfs=classifiers(data)
"""
